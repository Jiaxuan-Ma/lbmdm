...
def show():
    ...
    with col1:
        with st.expander("Hyper Parameter"):
            ...
            inputs['layer size'] = st.number_input('hidden layer size', 1, 20, 5)
            inputs['neuron size'] = st.number_input('neuron size', 5, 100, 15)
            ...
            inputs['activation'] = st.selectbox('activation',['relu'])
            # inputs['solver'] = st.selectbox('adam', ['lbfgs', 'sgd', 'adam'])
            inputs['solver'] = st.selectbox('solver',['adam'])
            ...
            inputs['max iter'] = st.number_input('max iter', 1, 500, 300)
